    ...
    <form method="POST" action="<?= htmlspecialchars($action) ?>">
        ...
        <div class="bg-white border rounded-lg p-6 mb-6">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-2" for="password">
                    <?= $isEdit ? 'New Password' : 'Password' ?> <?= $isEdit ? '' : '<span class="text-red-500">*</span>' ?>
                </label>
                <input type="password" 
                       id="password" 
                       name="password" 
        ...
        <div class="bg-white border rounded-lg p-6 mb-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-gray-700 font-semibold mb-2" for="role">User Role <span class="text-red-500">*</span></label>
                    <select id="role" name="role" required
                            class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
                        <option value="CUSTOMER" <?= ($user?->role->value === 'CUSTOMER') ? 'selected' : '' ?>>Customer</option>
                        <option value="EMPLOYEE" <?= ($user?->role->value === 'EMPLOYEE') ? 'selected' : '' ?>>Employee</option>
                ...
                <div class="flex flex-col gap-4">
                    <label class="flex items-center cursor-pointer">
                        <div class="relative">
                            <input type="checkbox" name="is_active" class="sr-only" <?= ($user?->is_active ?? true) ? 'checked' : '' ?>>
                            <div class="w-10 h-6 bg-gray-200 rounded-full transition-colors"></div>
                            <div class="dot absolute w-5 h-4 bg-white rounded-full shadow inset-y-2 left-1 top-1 transition-transform"></div>
                        </div>
                    ...
                    <label class="flex items-center cursor-pointer">
                        <div class="relative">
                            <input type="checkbox" name="is_verified" class="sr-only" <?= ($user?->is_verified ?? false) ? 'checked' : '' ?>>
...
input:checked ~ .bg-gray-200 { background-color: #DBEAFE; }
</style>