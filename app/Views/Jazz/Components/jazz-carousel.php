...
// Use the same day-color sequence as the schedule cards.
$dayPalette = ['lavender', 'pink', 'yellow', 'coral'];
$artistDayColors = [];
$dayIndex = 0;
...
foreach (($scheduleByDate ?? []) as $dateKey => $daySchedules) {
    ...
    $dayColor = $dayPalette[$dayIndex % count($dayPalette)];

    foreach ($daySchedules as $schedule) {
        $artistId = (int) ($schedule->artist_id ?? 0);
        if ($artistId > 0 && !isset($artistDayColors[$artistId])) {
            // Keep first occurrence so each artist has a stable day color.
            $artistDayColors[$artistId] = $dayColor;
        }
    }
...
<section class="py-10 bg-gray-50" aria-labelledby="artists-heading">
    <div class="container mx-auto px-4 max-w-[1600px]">
        <header class="mb-12">
            <h2 id="artists-heading" class="text-4xl md:text-5xl font-bold mb-2"
        ...
        <?php if (!empty($artists)): ?>
            <div class="relative">
                            ...
                            <div class="carousel-page" data-page="<?= $pageIndex ?>"
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 md:gap-8">
                                    ...
                                    <?php foreach ($pageArtists as $index => $artist):
                                        ...
                                        $cardColor = $artistDayColors[$artistId] ?? 'lavender';
                                    ?>
                ...
                <!-- Left Arrow -->
                <?php if ($totalPages > 1): ?>
                    <button type="button" id="prev-arrow" onclick="changePage(-1)"
                    ...
                    <!-- Right Arrow -->
                    <button type="button" id="next-arrow" onclick="changePage(1)"
