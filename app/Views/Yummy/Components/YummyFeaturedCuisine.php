...
// Try to get items from the featured cuisine section's gallery
if (!empty($featuredCuisineSection->gallery->media_items)) {
    $items = $featuredCuisineSection->gallery->media_items;
...
<section class="bg-[var(--yummy-sec-section)] text-[#FFFFFF] py-16 pt-4 pb-4 gap-6">
    ...
    <div class="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto px-6">
        ...
        <div class="bg-gray-800 p-4 rounded-lg">
            ...
            <img src="<?= htmlspecialchars($item->media->file_path) ?>" class="img rounded mb-4"
                style="height: 15rem; width: 25rem;"
                alt="<?= htmlspecialchars($item->media->alt_text ?? 'Featured Cuisine') ?>" />
