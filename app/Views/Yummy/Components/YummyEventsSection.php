<?php
namespace App\Views\Yummy\Components;
?>


<section class="bg-yellow-500 py-16 px-6">
      <h2 class="text-xl font-bold text-gray-900 mb-8 text-center">
        More Events to See
      </h2>
      <div class="grid md:grid-cols-4 gap-6 max-w-6xl mx-auto">
        <div class="bg-white rounded shadow text-center">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/b/b8/Fireworks_in_Haarlem.jpg"
            class="rounded-t"
            alt="Jazz Festival"
          />
          <div class="p-4">
            <h3 class="font-semibold">Jazz - Haarlem Festival</h3>
            <button
              class="bg-yellow-500 hover:bg-yellow-600 text-white text-sm px-4 py-2 rounded mt-3"
            >
              View more
            </button>
          </div>
        </div>
        <div class="bg-white rounded shadow text-center">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/e/e7/Theatre_in_Haarlem.jpg"
            class="rounded-t"
            alt="Magic Festival"
          />
          <div class="p-4">
            <h3 class="font-semibold">Magic - Haarlem Festival</h3>
            <button
              class="bg-yellow-500 hover:bg-yellow-600 text-white text-sm px-4 py-2 rounded mt-3"
            >
              View more
            </button>
          </div>
        </div>
        <div class="bg-white rounded shadow text-center">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/d/dd/Haarlem_Church.jpg"
            class="rounded-t"
            alt="History Festival"
          />
          <div class="p-4">
            <h3 class="font-semibold">History - Haarlem Festival</h3>
            <button
              class="bg-yellow-500 hover:bg-yellow-600 text-white text-sm px-4 py-2 rounded mt-3"
            >
              View more
            </button>
          </div>
        </div>
        <div class="bg-white rounded shadow text-center">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/9/93/Dance_event_Haarlem.jpg"
            class="rounded-t"
            alt="Dance Festival"
          />
          <div class="p-4">
            <h3 class="font-semibold">Dance - Haarlem Festival</h3>
            <button
              class="bg-yellow-500 hover:bg-yellow-600 text-white text-sm px-4 py-2 rounded mt-3"
            >
              View more
            </button>
          </div>
        </div>
      </div>
    </section>